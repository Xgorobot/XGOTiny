...
private:
    ...
    void InitializeIot() {
        ...
        mcp_server.AddTool("self.dog.action", 
            "设定动作ID表演不同动作,1为打招呼,2为撒娇,3为祈求/抬头,4为前后运动,5为左右摇摆,6为懊悔/生气,7为游泳,8为撒尿,9为伸懒腰,10为上下蹲起,11为摇头晃脑,12为坐下,13为挠痒,14为抱抱", 
            PropertyList({
                Property("ID", kPropertyTypeInteger, 0, 255),
            }), [this](const PropertyList& properties) -> ReturnValue {
        ...
        mcp_server.AddTool("self.laser.control",
            ...
            PropertyList({
                Property("mode", kPropertyTypeInteger, 0, 2),  // 参数名统一为 mode
            }),
            [this](const PropertyList& properties) -> ReturnValue {
